...
def iconf2crg(conformers: list) -> dict:
    ...
    return {conf.iconf: int(conf.crg) for conf in conformers}
...
def find_uniq_crgms_count_order2(
    crg_list_ms: list, begin_energy: float = None, end_energy: float = None
    ...
    Returns:
      A 3-tuple of lists:
        0: uniq_cms_data: list
        1: uniq_cms_states: list
        2: energy_diff_all: list
    ...
    if not begin_energy and not end_energy:
        ...
        # both energy bounds given; filter the input list energy accordingly:
        crg_list_ms = [[x[0], x[1], x[2]] for x in crg_list_ms
    ...
    for E, count, state in crg_list_ms:
        ...
        if k not in uniq_cms_data :
            ...
            # same state: increment the total count of ms
            uniq_cms_data[k][0] += count
            # get the maximum and minimum energy
            min_energy = min(min(uniq_cms_data[k][1]), E)
            max_energy = max(max(uniq_cms_data[k][1]), E)
            # reset the energy list with the min and max energy so far:
            uniq_cms_data[k][1].clear()
            uniq_cms_data[k][1].append(min_energy)
            uniq_cms_data[k][1].append(max_energy)

    logger.info(f"Number of unique charge ms: {len(uniq_cms_data):,}")
    # make a list of crgms counts, unique charge microstate, order and E difference.
    uniq_cms_states = []  # list of crg ms states
    energy_diff_all = []

    for k in uniq_cms_data:
        uniq_cms_states.append(list(k))
        # E list: holds single value for unique crg state, or [min, max]
        E = uniq_cms_data[k][1]
        if len(E) == 2:
            energy_diff_all.append(round(E[1] - E[0], 3))
    ...
    return uniq_cms_data, uniq_cms_states, energy_diff_all
